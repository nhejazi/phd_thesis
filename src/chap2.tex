...
discretizing the observed $A$ based on a number of bins $T$ and a binning
...
observed value $A$ falling in a given bin may be evaluated via standard
...
where the probability that a value of $A$ falls in a bin $[\alpha_{t-1},
...
/ (\alpha_t - \alpha_{t-1})$, for $\alpha_{t-1} \leq a < \alpha_t$, may be
constructed. As part of this procedure, the hazard estimates are mapped to
density estimates through rescaling of the estimates by the bin size ($\alpha_t
- \alpha_{t-1}$).
